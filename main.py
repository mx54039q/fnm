    ...
    with tf.Session(config=config, graph=net.graph) as sess:
        ...
        if 0:
            for step in range(num_batch):
                ...
                if step == num_batch - 1:
                    saver.save(sess, cfg.logdir + '/session01/session01_1-%04d-%02d' % (0, global_step))#
        # 2. Join GAN loss
        for epoch in range(cfg.epoch):
            for step in range(num_batch):
                ...
                if (global_step + 1) % cfg.test_sum_freq == 0:
                    ...
                    dl, gl, fl, images = sess.run([net.front_loss,net.feature_loss,net.texture],
                        {net.profile:te_profile, net.front:te_front, net.is_train:False})
                ...
                if step == num_batch - 1:
                    saver.save(sess, cfg.logdir + '/session01/session01_1-%04d-%02d' % (epoch, global_step))#
