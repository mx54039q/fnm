...
class Net(object):
    ...
    def decoder(self, feature, y=None, reuse=False):
        ...
        with tf.variable_scope('decoder', reuse=reuse) as scope:            
            ...
            with tf.variable_scope('dconv5_concat'):
                fc1 = fullyConnect(feature, 56 * 56, 'fc1')
        ...
        with tf.variable_scope("discriminator", reuse=reuse) as scope:
            ...
            with tf.variable_scope("dis_0"):
                ...
                with tf.variable_scope('d_conv5'):
                    d0_h5 = conv2d(d0_h4, 256, 'd_conv5', kernel_size=4, strides=2)
                # h5 is (4 x 4 x 1)
            ...
            with tf.variable_scope("dis_1"):
                ...
                # h3 is (7 x 7 x 256)
                with tf.variable_scope('d_conv4'):
                    d1_h4 = conv2d(d1_h3, 256, 'd_conv4', kernel_size=4, strides=2)
                # h4 is (4 x 4 x 1)
            ...
            with tf.variable_scope("dis_2"):
                ...
                # h2 is (7 x 7 x 128)
                with tf.variable_scope('d_conv3'):
                    d2_h3 = conv2d(d2_h2, 128, 'd_conv3', kernel_size=4, strides=2)
                # h3 is (4 x 4 x 1)
    ...
    def loss(self):
        with tf.name_scope('loss') as scope:
            ...
            # 4. Adversarial Loss
            self.d_loss_real_56 = tf.reduce_mean(tf.losses.mean_squared_error(predictions=self.dr_56_logits, 
                                                                                 labels=tf.ones_like(self.dr_56)))
            self.d_loss_fake_56 = tf.reduce_mean(tf.losses.mean_squared_error(predictions=self.df_56_logits, 
                                                                                 labels=tf.zeros_like(self.df_56)))
            self.d_loss_real_112 = tf.reduce_mean(tf.losses.mean_squared_error(predictions=self.dr_112_logits, 
                                                                                 labels=tf.ones_like(self.dr_112)))
            self.d_loss_fake_112 = tf.reduce_mean(tf.losses.mean_squared_error(predictions=self.df_112_logits, 
                                                                                 labels=tf.zeros_like(self.df_112)))
            self.d_loss_real_224 = tf.reduce_mean(tf.losses.mean_squared_error(predictions=self.dr_224_logits, 
                                                                                 labels=tf.ones_like(self.dr_224)))
            self.d_loss_fake_224 = tf.reduce_mean(tf.losses.mean_squared_error(predictions=self.df_224_logits, 
            ...
            self.g_loss_56 = tf.reduce_mean(tf.losses.mean_squared_error(predictions=self.df_56_logits, 
                                                                                 labels=tf.ones_like(self.df_56)))
            self.g_loss_112 = tf.reduce_mean(tf.losses.mean_squared_error(predictions=self.df_112_logits, 
            ...
            self.g_loss_224 = tf.reduce_mean(tf.losses.mean_squared_error(predictions=self.df_224_logits, 
                                                                                 labels=tf.ones_like(self.df_224)))
            ...
            self.gen_loss = cfg.lambda_l1 * self.front_loss + cfg.lambda_fea * self.feature_loss + \
                            cfg.lambda_gan * self.g_loss + cfg.lambda_reg * self.l2_loss + cfg.lambda_sym * self.sym_loss
            self.dis_loss = cfg.lambda_gan * self.d_loss + cfg.lambda_reg * self.l2_loss + cfg.lambda_sym * self.sym_loss
