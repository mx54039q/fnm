...
class WGAN_GP(object):
    ...
    3. 对应G有三个尺度的独立的判别器, 输入先进行减均值归一化, PN和LReLU, 第一层和最后一层不用PN, 全卷积网络(k4s2)
    ...
    def build_arch(self):
        ...
        # Gradient Penalty
        def interpolate(gen_front, gt_front):
            alpha = tf.random_uniform(shape=gt_front.get_shape(), minval=0.,maxval=1.)
            differences = gen_front - gt_front
            return gt_front + (alpha * differences)
        with tf.name_scope('gp'):
            inter_56 = interpolate(self.texture_56, self.resized_56)
            inter_112 = interpolate(self.texture_112, self.resized_112)
            inter_224 = interpolate(self.texture_224, self.front)
            _, D_inter_56, _, D_inter_112, _, D_inter_224 = self.discriminator(inter_56, \
                                                            inter_112, inter_224, reuse=True)
            grad_56,grad_112,grad_224 = tf.gradients([D_inter_56,D_inter_112,D_inter_224], 
                                                     [inter_56,inter_112,inter_224])
            slopes_56 = tf.sqrt(tf.reduce_sum(tf.square(grad_56), reduction_indices=range(1, inter_56.shape.ndims)))
            slopes_112 = tf.sqrt(tf.reduce_sum(tf.square(grad_112), reduction_indices=range(1, inter_112.shape.ndims)))
            slopes_224 = tf.sqrt(tf.reduce_sum(tf.square(grad_224), reduction_indices=range(1, inter_224.shape.ndims)))
            self.gradient_penalty = tf.reduce_mean((slopes_224 - 1.) ** 2)
                                    tf.reduce_mean((slopes_56 - 1.) ** 2) + \
                                    tf.reduce_mean((slopes_112 - 1.) ** 2)
        ...
        with tf.variable_scope("discriminator", reuse=reuse) as scope:
            with tf.variable_scope("dis_0"):
                ...
                with tf.variable_scope('d_conv1'):
                    d0_h1 = lrelu(ins_norm(conv2d(d0_h0, 64, 'd_conv1', kernel_size=4, strides=2)))
                # h1 is (56 x 56 x 64)
                with tf.variable_scope('d_conv2'):
                    d0_h2 = lrelu(ins_norm(conv2d(d0_h1, 128, 'd_conv2', kernel_size=4, strides=2)))
                # h2 is (28 x 28 x 128)
                with tf.variable_scope('d_conv3'):
                    d0_h3 = lrelu(ins_norm(conv2d(d0_h2, 256, 'd_conv3', kernel_size=4, strides=2)))
                # h3 is (14 x 14 x 256)
                with tf.variable_scope('d_conv4'):
                    d0_h4 = lrelu(ins_norm(conv2d(d0_h3, 256, 'd_conv4', kernel_size=4, strides=2)))
                # h4 is (7 x 7 x 256)
                with tf.variable_scope('d_conv5'):
                    d0_h5 = lrelu(ins_norm(conv2d(d0_h4, 256, 'd_conv5', kernel_size=4, strides=2)))
                # h5 is (4 x 4 x 256)
            ...
            with tf.variable_scope("dis_1"):
                ...
                # h0 is (56 x 56 x 32)
                with tf.variable_scope('d_conv1'):
                    d1_h1 = lrelu(ins_norm(conv2d(d1_h0, 64, 'd_conv1', kernel_size=4, strides=2)))
                # h1 is (28 x 28 x 64)
                with tf.variable_scope('d_conv2'):
                    d1_h2 = lrelu(ins_norm(conv2d(d1_h1, 128, 'd_conv2', kernel_size=4, strides=2)))
                # h2 is (14 x 14 x 128)
                with tf.variable_scope('d_conv3'):
                    d1_h3 = lrelu(ins_norm(conv2d(d1_h2, 256, 'd_conv3', kernel_size=4, strides=2)))
                # h3 is (7 x 7 x 256)
                with tf.variable_scope('d_conv4'):
                    d1_h4 = lrelu(ins_norm(conv2d(d1_h3, 256, 'd_conv4', kernel_size=4, strides=2)))
                # h4 is (4 x 4 x 256)
            ...
            with tf.variable_scope("dis_2"):
                ...
                # h0 is (28 x 28 x 32)
                with tf.variable_scope('d_conv1'):
                    d2_h1 = lrelu(ins_norm(conv2d(d2_h0, 64, 'd_conv1', kernel_size=4, strides=2)))
                # h1 is (14 x 14 x 64)
                with tf.variable_scope('d_conv2'):
                    d2_h2 = lrelu(ins_norm(conv2d(d2_h1, 128, 'd_conv2', kernel_size=4, strides=2)))
                # h2 is (7 x 7 x 128)
                with tf.variable_scope('d_conv3'):
                    d2_h3 = lrelu(ins_norm(conv2d(d2_h2, 256, 'd_conv3', kernel_size=4, strides=2)))
                # h3 is (4 x 4 x 256)
    ...
    def loss(self):
        ...
        with tf.name_scope('loss') as scope:
            ...
            with tf.name_scope('Adversarial_Loss'):
                ...
                self.d_loss = tf.reduce_mean(self.df_224_logits) - tf.reduce_mean(self.dr_224_logits)
            ...
            # 5. Gradient Penalty. (Done in function 'build_arch')
